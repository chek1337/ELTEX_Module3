...
int main()
{
    ...
    mqd_t mq_id_user100 = mq_open(PATHNAME1, O_CREAT | O_RDWR, 0666, &queue_attr);
    if(mq_id_user100 == -1)
        myperr("mq_open1");

    mqd_t mq_id_user200 = mq_open(PATHNAME2, O_CREAT | O_RDWR, 0666,  &queue_attr);
    if(mq_id_user200 == -1)
        myperr("mq_open2");
    ...
    for ( int i = 0; ; i++)
    {
        ...
        if(mq_send(mq_id_user200, snd_buf, MSG_SIZE, 100) == -1)
            myperr("mq_send");
        if(mq_receive(mq_id_user100, rcv_buf, MSG_SIZE , NULL) == -1)
            myperr("mq_receive");
